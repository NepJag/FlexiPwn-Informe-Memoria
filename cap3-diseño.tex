...
\begin{itemize}
    ...
    2023"}) y \texttt{process\_id} (hash SHA-256 truncado a 12 caracteres de la
    combinación \texttt{pid:cmd}, que identifica unívocamente al proceso incluso
    ante reutilización de PID).
\end{itemize}
...
\begin{sloppypar}
    \begin{itemize}
        ...
        archivo de log en el host) y \texttt{parsed} (objeto JSON completo de la
        línea detectada).
    \end{itemize}
\end{sloppypar}
...
\begin{itemize}
    ...
    ASCII imprimible extraído del payload), \texttt{src\_ip},
    \texttt{src\_port}, \texttt{dst\_ip}, \texttt{dst\_port} y \texttt{flags}
    (banderas TCP del paquete, por ejemplo \texttt{[P.]} o \texttt{[S.]}).
    ...
    [S.]}). \texttt{details} contiene \texttt{dst\_ip} y \texttt{dst\_port}.
\end{itemize}
...
educador y qué queda únicamente en log y en las vistas agregadas \texttt{run
dashboard} y \texttt{run feed}.
...
\begin{description}
    ...
    \texttt{field\_matches} con la clave \texttt{data}; el matching se realiza
    con \texttt{re.IGNORECASE} sobre el texto extraído del paquete. Requiere que
    el escenario defina \texttt{attacker\_image} y, opcionalmente,
...
aceptable para el contexto educativo. La validación de unicidad sobre el par
...
\textit{match} serializado como JSON. Cuando un target de primer nivel es un
nodo lógico (\texttt{and}, \texttt{or} o \texttt{not}), se persiste un
\texttt{TargetResult} por cada hoja del árbol además del nodo raíz, y
el campo \texttt{children} reconstruye la estructura lógica para que la Capa 4
pueda presentar al educador qué sub-condición disparó el \textit{match}. Tiene
índice ordinario sobre \texttt{run\_id} para recuperar eficientemente todos los
...
crean con permisos \texttt{0o700}, accesibles únicamente por el proceso del
daemon rootless. Al destruir el entorno, el árbol de directorios completo se
elimina mediante \texttt{shutil.rmtree()}. Los directorios de captura de red,
cuando existen, se eliminan en el mismo ciclo de destrucción, antes de detener
los contenedores.
