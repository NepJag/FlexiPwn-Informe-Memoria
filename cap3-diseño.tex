...
nueva clase, sin tocar el motor de evaluación ni la CLI de
...
agregar el despliegue de un contenedor \textit{sidecar} de captura al
...
monitor (caso de MySQL con \texttt{general\_log}) y, cuando se quiere
inspeccionar el \textit{payload} SQL a nivel de red, que la conexión
...
de datos que no exponga \textit{logs} equivalentes obligaría al educador a
definir condiciones de éxito por otras señales. Esta dependencia es propia del
dominio
...
entradas de \textit{log} de aplicación y tráfico de red observado. Su
comportamiento es
cercano al de un SIEM acotado a un único escenario de ataque.
...
\begin{itemize}
    ...
    \textit{kernel} del \textit{host} sin ejecutar nada dentro del contenedor.

    \item \textbf{Logs de aplicación}: mediante lectura directa de archivos de
    \textit{log} en el \textit{host}, escritos por el contenedor a través de
    volúmenes montados en rutas explícitamente definidas en el YAML del
    escenario.
    ...
    mount} del \textit{host}. El monitor de red lee ese archivo desde fuera del
...
\begin{figure}[h]
    \centering
    \resizebox{\textwidth}{!}{\input{diagramas/contrato-12.tex}}
    \caption{Contrato Capa 1 $\rightarrow$ Capa 2: estado observable de solo
    lectura.}
    \label{fig:contrato-12}
\end{figure}

Este contrato se completa con lo que el educador debe garantizar en la imagen
que entrega. Para que un sistema vulnerable resulte observable sin
modificarlo, la imagen debe cumplir con lo siguiente:

\begin{itemize}
    \item \textbf{Punto de entrada alcanzable.} Debe exponer un servicio al que
    el estudiante llegue desde el contenedor atacante, como un servidor SSH o
    una aplicación web.

    \item \textbf{Arranque observable.} Debe declarar un \texttt{HEALTHCHECK} o
    admitir un tiempo de espera de inicio suficiente, porque los monitores
    toman una línea base al crear el entorno y un servicio a medio inicializar
    la contamina.

    \item \textbf{Estado inicial estable.} Los procesos y archivos que cambian
    por sí solos, sin intervención del estudiante, producen eventos que el
    motor no distingue de un avance real.

    \item \textbf{Registros en archivos declarados.} Si el escenario define
    objetivos sobre registros, el servicio debe escribirlos en archivos cuyas
    rutas se declaren en la descripción del escenario, y el directorio que los
    contiene debe existir en la imagen con permisos de escritura. Ese
    directorio se monta desde el anfitrión, de modo que su contenido queda
    fuera de la capa de escritura del contenedor y no alimenta los objetivos de
    sistema de archivos.

    \item \textbf{Tráfico legible.} Si el escenario define objetivos de red, el
    tráfico relevante debe viajar en claro, sin TLS, por la red del escenario.

    \item \textbf{Sin privilegios especiales.} La imagen se ejecuta bajo un
    \textit{runtime} sin privilegios, de modo que no puede depender de
    capacidades adicionales, del modo privilegiado ni del socket del
    \textit{daemon} de contenedores.
\end{itemize}

El repositorio del proyecto documenta estos requisitos junto a las imágenes de
referencia que los ilustran~\cite{flexipwn}.
...
\begin{itemize}
    ...
    \textit{target} correspondiente en el schema YAML: ningún escenario del
    sistema
...
Generados por el monitor de \textit{logs} mediante lectura directa de archivos
en el \textit{host} escritos a través de volúmenes Docker.
\begin{sloppypar}
    \begin{itemize}
        \item \textbf{\texttt{log\_entry}}: línea de \textit{log} nueva
        detectada en un archivo monitoreado. El monitor autodetecta el formato
        de cada línea, lo que produce dos variantes de \texttt{details}. Si la
        línea parsea como JSON, contiene \texttt{source\_file} (ruta del archivo
        de \textit{log} en el \textit{host}) y \texttt{parsed} (objeto JSON
        completo de la línea). Si no, la
...
\begin{itemize}
    ...
    ASCII imprimible extraído del \textit{payload}, la captura se realiza a
    nivel de paquete con \texttt{tcpdump -A}, y el parser observa además las
    banderas TCP del paquete, por ejemplo \texttt{[S.]} o \texttt{[P.]}, para
    clasificarlo),
...
\begin{figure}[h]
    \centering
    \resizebox{\textwidth}{!}{\input{diagramas/contrato-23.tex}}
    \caption{Contrato Capa 2 $\rightarrow$ Capa 3: eventos estructurados
    (\texttt{MonitorEvent}).}
    \label{fig:contrato-23}
\end{figure}
...
éxito mediante dos estructuras \texttt{Pydantic} complementarias.
...
El campo \texttt{children} se utiliza únicamente cuando el \textit{target} es un
nodo lógico (\texttt{and}, \texttt{or} o \texttt{not}). Contiene los
\texttt{TargetResult} de los sub-\textit{targets} evaluados recursivamente, lo
que permite a la Capa 4 mostrar al educador no solo qué rama lógica matcheó,
sino qué sub-condiciones internas fueron las que la satisficieron.

\texttt{EvaluationResult} agrega el estado de todos los \textit{targets} de un
...
\emph{hojas} matcheadas y el total de hojas del árbol de \textit{targets} del
escenario. Las hojas son los \textit{targets} atómicos (\texttt{file\_created},
...
\texttt{all}) se satisface con el conjunto de \textit{targets} de primer nivel
marcados como \texttt{matched}. Cuando un \textit{target} de primer nivel es un
nodo lógico, su
...
un \textit{target} cambia de estado o cuando \texttt{completed} pasa de
\texttt{False} a \texttt{True}. Los logros son irreversibles dentro de un
\textit{run}: una vez que \texttt{matched} es \texttt{True}, no vuelve a
\texttt{False}. La Capa 4
...
educador y qué queda únicamente en \textit{log} y en las vistas agregadas
\texttt{dashboard} y \texttt{feed}. A partir de los instantes en que cada hoja
matchea, la Capa 4 deriva un registro de tiempos por \textit{run}: la duración
total y el
...
\begin{figure}[h]
    \centering
    \resizebox{\textwidth}{!}{\input{diagramas/contrato-34.tex}}
    \caption{Contrato Capa 3 $\rightarrow$ Capa 4: resultados de evaluación.}
    \label{fig:contrato-34}
\end{figure}
...
\begin{figure}[h]
    \centering
    \resizebox{\textwidth}{!}{\input{diagramas/contrato-41.tex}}
    \caption{Contrato Capa 4 $\rightarrow$ Capa 1: ciclo de vida vía
    \texttt{EnvironmentProvider}.}
    \label{fig:contrato-41}
\end{figure}
...
parsea y valida con \texttt{Pydantic v2} antes de cualquier ejecución. Este
archivo es la
...
environment:
  ...
  attacker_image: str    # imagen Docker para Capa 1b (requerido)
...
Los tipos de \textit{target} disponibles se dividen en \emph{tipos atómicos},
que evalúan una condición concreta sobre eventos de la Capa 2, y \emph{nodos
lógicos}, que componen otros \textit{targets} en árboles recursivos.
...
\begin{description}
    ...
    \item[\texttt{log\_pattern}] Detecta una entrada de \textit{log} que cumpla
    ...
    matchear contra la línea completa cuando el \textit{log} es texto plano.
    Requiere que
    \texttt{environment.log\_paths} declare al menos una ruta.

    \item[\texttt{network\_payload}] Detecta \textit{payloads} de paquetes de
    red cuyo contenido ASCII imprimible coincida con una expresión regular.
    Requiere \texttt{field\_matches} con la clave \texttt{data}, y el matching
    se realiza
    ...
    contenedor vulnerable. Opcionalmente se acota la captura con
    \texttt{capture\_filter} en el bloque \texttt{environment}.

    \item[\texttt{network\_connection}] Detecta conexiones TCP establecidas
    (paquete \texttt{SYN-ACK}) hacia un destino. Requiere \texttt{dst\_port}, y
    acepta \texttt{dst\_ip} para restringir a una IP específica.
\end{description}

\paragraph{Nodos lógicos}\mbox{}\\[1.0em]
Los nodos lógicos componen otros \textit{targets} en árboles recursivos y se
identifican por su \texttt{type}. Sus sub-\textit{targets} se definen en el
campo \texttt{targets}, que puede contener tanto tipos atómicos como otros nodos
lógicos.

\begin{description}
    \item[\texttt{and}] Matchea cuando todos sus sub-\textit{targets} matchean.
    Requiere al menos dos sub-\textit{targets}.

    \item[\texttt{or}] Matchea cuando al menos un sub-\textit{target} matchea.
    Requiere al menos dos sub-\textit{targets}.

    \item[\texttt{not}] Matchea cuando su único sub-\textit{target} no matchea.
    Requiere exactamente un sub-\textit{target}. No puede aparecer como
    \textit{target} de primer nivel del escenario, ya que un escenario que se
    completara por la ausencia de un evento podría darse por exitoso sin acción
...
como azúcar sintáctica para listas planas de \textit{targets} atómicos: es
...
\begin{figure}[h]
    \centering
    \resizebox{\textwidth}{!}{\input{diagramas/contrato-ed3.tex}}
    \caption{Contrato Educador $\rightarrow$ Capa 3: carga y validación del
    escenario YAML.}
    \label{fig:contrato-ed3}
\end{figure}
...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{\input{diagramas/erd.tex}}
...
mediante \texttt{SQLModel}. La elección de SQLite responde al contexto de
despliegue: una
...
datos. La contraseña se persiste como \textit{hash} \texttt{bcrypt}, el texto
plano solo
...
frecuentes (\textit{runs} por participante, \textit{runs} por escenario), y un
índice único sobre \texttt{env\_id} para garantizar que el identificador de
entorno expuesto al participante no se repita entre \textit{runs}. Persiste
además los datos efímeros de
...
transiciones mediadas por el \textit{daemon}. La validación de unicidad sobre el par
(\texttt{scenario\_id}, \texttt{participant\_id}) para \textit{runs} activos se
hace a nivel aplicación, no de base de datos: el mismo participante puede tener
múltiples \textit{runs} históricos del mismo escenario, lo que permite
reintentos sin
borrar el historial previo.

\begin{figure}[H]
    \centering
    \resizebox{0.7\textwidth}{!}{\input{diagramas/run-estados.tex}}
    \caption{Máquina de estados de un \textit{run} (\texttt{RunStatus}), con las
    transiciones gestionadas por el \textit{daemon}.}
...
del árbol de \textit{targets}: el árbol del escenario se aplana en orden DFS y
cada hoja recibe un \texttt{target\_index} secuencial. Los nodos lógicos
(\texttt{and}, \texttt{or}, \texttt{not}) no se persisten como filas, aportan
estructura solo en memoria. El reenlace entre la hoja del modelo
\texttt{Pydantic} de la Capa 3 y su fila es posicional por
\texttt{target\_index} en ese mismo orden DFS, lo que evita
...
recuperar eficientemente todos los resultados de un \textit{run}.

\begin{sloppypar}
\textbf{RunEvent} guarda el flujo completo de eventos del monitor asociados a un
\textit{run} (\texttt{monitor\_type}, \texttt{event\_type},
\texttt{details\_json}, \texttt{timestamp}). Sirve como bitácora auditable del
ejercicio, independiente de qué \textit{targets} matcheen o no: permite
reconstruir a posteriori la secuencia de
...
\begin{figure}[h]
    \centering
    \resizebox{0.7\textwidth}{!}{\input{diagramas/topologia.tex}}
    \caption[Topología de red del entorno]{Topología de red del entorno: red interna aislada
    (\texttt{internal=True}) y red externa (bridge), con el atacante en ambas
    redes, el vulnerable solo en la interna, y el \textit{sidecar} compartiendo
    el \textit{netns} del vulnerable.}
...
conecta exclusivamente el contenedor atacante al \textit{host}. Esto permite que
el participante acceda al entorno atacante mediante SSH desde su máquina,
...
del escenario sean alcanzables desde el \textit{host}. El contenedor vulnerable
solo tiene presencia en la red interna.

El contenedor \textit{sidecar} de captura de red (Sección~\ref{sec:capa2}) no se
...
proceso del \textit{daemon} \textit{rootless}. La base de volúmenes y el
directorio de captura de red usan ese modo. Una excepción son los directorios de
\textit{logs} que se montan al contenedor vulnerable, creados con \texttt{0o777}
para que el proceso del contenedor pueda escribir sus archivos de \textit{log}.
Al destruir el entorno, el árbol
