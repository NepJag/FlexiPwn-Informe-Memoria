...
producción. Para ello, se diseña y desarrolla un sistema de evaluación
...
reversa. Se entenderá por validación automática aquella que marca el ejercicio
...
inteligencia artificial se emplea como herramienta de apoyo y no se reconoce
como coautoría.
...
en sesiones anteriores), seguido de lo que hay que hacer y de los puntos que
vale la pena discutir, pidiendo siempre alternativas para elegir la mejor
implementación. Se cerraba con algún criterio de validación y con notas que
precisan la ejecución. A mitad de una tarea el patrón cambiaba: se revisaba lo
...
el resultado es satisfactorio. La mayoría de veces, se recordaba al modelo lo
...
se requería y un prompt de traspaso que oriente a la sesión siguiente.

A modo ilustrativo, dos ejemplos representativos del tipo de instrucción
entregada:

\begin{itemize}
    \item Un prompt que funciona bien: ``Retoma el informe: primero una foto del
    ...
    presente y opciones ante cada alternativa.'' Funciona porque aporta
    contexto, descompone la tarea, exige validación y opciones, y fija
    restricciones explícitas.
    \item Un prompt que funciona mal: una instrucción amplia y sin contexto como
    ``mejora el capítulo de implementación'', que al no acotar el objetivo ni
    los criterios de aceptación produce cambios genéricos o de alcance excesivo
    que luego exigen una corrección considerable. La lección es que los prompts
    pobres en contexto y sin criterios claros rinden resultados de bajo valor.
\end{itemize}
...
traspasos entre sesiones, que resultan clave para mantener la coherencia a lo
